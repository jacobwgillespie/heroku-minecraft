class Sync
  ...
  WORLD = ['world']
  ADMIN = ['server.properties', 'banned-ips.txt', 'banned-players.txt', 'ops.txt', 'white-list.txt']
  ...
  def self.push_to_s3(filename)
    `boto-rsync -a $AWS_KEY -s $AWS_SECRET /app/#{filename} s3://$S3_BUCKET/#{filename}`
  end

  def self.pull_from_s3(filename)
    `boto-rsync -a $AWS_KEY -s $AWS_SECRET s3://$S3_BUCKET/#{filename} /app/#{filename}`
  end
